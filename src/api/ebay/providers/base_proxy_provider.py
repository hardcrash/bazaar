...
class BaseProxyProvider:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.provider_cfg = kwargs.get("provider_cfg") or kwargs.get("provider_config") or {}
    
        # 2. Extract shared core global application config runtime state safely
        self.config: Any = kwargs.get("config") if kwargs.get("config") is not None else {}
        ...
        if not isinstance(self.provider_cfg, dict):
            self.provider_cfg = dict(self.provider_cfg.__dict__) if hasattr(self.provider_cfg, '__dict__') else {}
